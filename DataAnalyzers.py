...
def set_blue_and_red_triangle(xlims, ylims, up_or_down):
    ...
    Returns
    -------
    Coordinates for the triangles, to place triangles in the bottom left 
    of the graph
    """
    if up_or_down == "up":
...
def plot_stocks_data(df, name):
    ...
    df = df.reset_index() #  i need to reset index since i want to use Date a
    
    #PLOTTING
    mpl.style.use('ggplot')# as column 
    fig, ax = plt.subplots()
